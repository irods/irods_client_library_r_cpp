...
guiProgressCallback
r_put_iCommandProgStat( operProgress_t *operProgress ) {
		...
		)
{
    ...
    // Legacy of the iput icommand logic
    ...
    if( !data_type.empty() )
    	myRodsArgs.dataTypeString = (char*)data_type.c_str();
    if( !metadata.empty() )
    	myRodsArgs.metadata_string = (char*)metadata.c_str();
    if( !acl.empty() )
    	myRodsArgs.acl_string = (char*)acl.c_str();
    ...
    if ( myRodsArgs.progressFlag == True ) {
        gGuiProgressCB = ( guiProgressCallback ) r_put_iCommandProgStat;
    }

    status = putUtil( &conn, &myEnv, &myRodsArgs, (rodsPathInp_t*)(rodsPathInp.get()) );
    ...
    if ( status < 0 ) {
      ...
      if(rodsPathInp->destPath->objType == COLL_OBJ_T){
