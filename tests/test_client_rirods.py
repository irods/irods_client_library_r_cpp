...
def create_test(r_test_script):
    def test(self):
        subprocess.call(['irule', "writeLine('serverLog','=======[ BEGIN ]=====[ {0} ]')".format(r_test_script), 'null','null'])
        ...
        subprocess.call(['irule', "writeLine('serverLog','=========[ END ]=====[ {0} ]')".format(r_test_script), 'null','null'])
    return test
